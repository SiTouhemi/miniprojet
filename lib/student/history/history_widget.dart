...
class _HistoryWidgetState extends State<HistoryWidget> {
  ...
  Widget _buildTimeSlotPickerDialog(
      ReservationRecord reservation, DateTime date) {
    return AlertDialog(
  ...
  Future<void> _processModification(
      ReservationRecord reservation, TimeSlotRecord newSlot) async {
    ...
    try {
      ...
      if (result['success'] == true) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(result['error'] ?? 'Échec de la modification'),
  ...
  Future<void> _handleCancelReservation(ReservationRecord reservation) async {
    ...
    try {
      ...
      if (result['success'] == true) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(result['error'] ?? 'Échec de l\'annulation'),
              backgroundColor: Colors.red),
        );
      }
    } catch (e) {
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Container(
          width: 250,
          height: 250,
          child: reservation.qrCode.isNotEmpty
              ? QrImageView(
                        ...
                        builder: (context, snapshot) {
                          ...
                          if (_selectedFilterIndex == 0) {
                            ...
                            filteredReservations.sort(
                                (a, b) => a.creneaux!.compareTo(b.creneaux!));
                          } else {
                            // Past
                            filteredReservations = allReservations.where((r) {
                              return r.creneaux?.isBefore(now) ??
                                  true; // or status is used/cancelled
                            }).toList();
                          }
  ...
  Widget _buildReservationCard(ReservationRecord reservation) {
    final bool isUpcoming = reservation.creneaux != null &&
        reservation.creneaux!.isAfter(DateTime.now());
    final bool isConfirmed = reservation.status == 'confirmed';
    final bool canModify = isUpcoming &&
        isConfirmed &&
        (reservation.creneaux!.difference(DateTime.now()).inHours >= 2);
    ...
    return Container(
      decoration: BoxDecoration(
          ...
          border: Border.all(
            color: isUpcoming
                ? Color(0xFF1C1284).withOpacity(0.1)
                : Colors.transparent,
          )),
      child: Padding(
        padding: EdgeInsets.all(16.0),
                ...
                if (reservation.qrCode.isNotEmpty && isConfirmed)
                  GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        color: Color(0xFF1C1284).withOpacity(0.1),
                        borderRadius: BorderRadius.circular(8.0),
                      ),
                      child: Icon(Icons.qr_code_2,
                          size: 32, color: Color(0xFF1C1284)),
                    ),
                  ),
              ],
            ),
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          Text(text,
              style:
                  GoogleFonts.inter(color: Color(0xFF7F8C8D), fontSize: 14.0)),
        ],
      ),
    );
  }
