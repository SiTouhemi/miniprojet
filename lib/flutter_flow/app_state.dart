...
class FFAppState extends ChangeNotifier {
  ...
  final SyncPerformanceMonitor _performanceMonitor =
      SyncPerformanceMonitor.instance;
  ...
  void _setupUserDataListener(String uid) {
    ...
    final operationId = _performanceMonitor
        .startSyncOperation('user_data_sync', metadata: {'uid': uid});
        ...
        if (snapshot.exists) {
          try {
            ...
            if (_currentUser != null) {
              ...
              final ticketsChanged =
                  _currentUser!.tickets != updatedUser.tickets;
              ...
              if (balanceChanged) {
                AppLogger.sync('User balance updated: ${updatedUser.pocket} DT',
                    success: true);
              }
              if (ticketsChanged) {
                AppLogger.sync('User tickets updated: ${updatedUser.tickets}',
                    success: true);
              }
              if (roleChanged) {
                AppLogger.i('User role changed to: ${updatedUser.role}',
                    tag: 'FFAppState');
              }
              ...
              _performanceMonitor.completeSyncOperation(operationId,
                  success: true, recordsProcessed: 1);
            ...
            _handleSyncError(
                'user_data_sync', 'Error updating user data: $e', operationId);
          }
        } else {
          // User document was deleted - handle this case
          _handleSyncError('user_data_sync',
              'User document not found. Please contact support.', operationId);
        }
      },
      onError: (error) {
        _handleSyncError(
            'user_data_sync', 'User data sync error: $error', operationId);
      },
    );
  }
  ...
  void _setupReservationsListener(String uid) {
    ...
    final operationId = _performanceMonitor
        .startSyncOperation('reservations_sync', metadata: {'uid': uid});
    ...
    _reservationsSubscription = FirebaseFirestore.instance
      ...
      (snapshot) {
        try {
          ...
          _performanceMonitor.completeSyncOperation(operationId,
              success: true, recordsProcessed: snapshot.docs.length);
          _syncFailureCounts.remove('reservations_sync');

          notifyListeners();
        } catch (e) {
          _handleSyncError('reservations_sync',
              'Error updating reservations: $e', operationId);
        }
      },
      onError: (error) {
        _handleSyncError('reservations_sync', 'Reservations sync error: $error',
            operationId);
      },
    );
  }
  ...
  void _setupMenuListener() {
    ...
    final operationId = _performanceMonitor.startSyncOperation('menu_sync',
        metadata: {'date': DateTime.now().toIso8601String()});

    _menuSubscription = MenuService.instance.getTodaysMenuStream().listen(
      (menus) {
        try {
          _todaysMenu = menus;
          _clearError();
          ...
          _performanceMonitor.completeSyncOperation(operationId,
              success: true, recordsProcessed: menus.length);
  ...
  Future<void> _validateUserDataConsistency(UserRecord user) async {
    try {
      final validation =
          await _validationService.validateUserData(user, user.uid);
      if (!validation.isValid) {
        AppLogger.w('User data validation failed: ${validation.errors}',
            tag: 'FFAppState');
        // Log validation errors but don't show to user unless critical
        if (validation.errors.any(
            (error) => error.contains('Balance') || error.contains('Role'))) {
          _setLastError(
              'Incohérence détectée dans les données. Actualisation en cours...');
  ...
  Future<void> _validateAllData() async {
    ...
    try {
      ...
      AppLogger.e('Comprehensive data validation error',
          error: e, tag: 'FFAppState');
    }
  }
  ...
  Future<void> retryConnection() async {
    try {
      ...
      AppLogger.i('Connection restored and listeners re-established',
          tag: 'FFAppState');
    } catch (e) {
  ...
  Future<void> initializeUserSync(String uid) async {
    try {
      ...
      if (userDoc.exists) {
        ...
        AppLogger.i('User data synchronization initialized for: ${user.nom}',
            tag: 'FFAppState');
      } else {
  ...
  Future<void> refreshUserData() async {
    ...
    final operationId = _performanceMonitor.startSyncOperation(
        'user_data_refresh',
        metadata: {'uid': _currentUser!.uid});
    ...
    try {
      ...
      if (userDoc.exists) {
        ...
        final validation =
            _validationService.validateNoHardcodedData(updatedUser);
        if (!validation.isValid) {
          AppLogger.w(
              'Potentially hardcoded data detected: ${validation.errors}',
              tag: 'FFAppState');
        }
  ...
  Future<void> loadTimeSlots(DateTime date) async {
    ...
    try {
      ...
      _timeSlotsSubscription =
          TimeSlotService.instance.getAvailableTimeSlotsStream(date).listen(
        (timeSlots) {
          ...
          _performanceMonitor.completeSyncOperation(operationId,
              success: true, recordsProcessed: timeSlots.length);
          ...
          AppLogger.sync(
              'Time slots updated: ${timeSlots.length} slots available',
              success: true,
              recordCount: timeSlots.length);
        },
        onError: (error) {
          _handleSyncError(
              'time_slots_sync', 'Time slots sync error: $error', operationId);
          _isLoadingTimeSlots = false;
          notifyListeners();
        },
      );
  ...
  Future<void> loadUserReservations() async {
    ...
    try {
      ...
      try {
        _userReservations =
            await AppService.instance.getUserReservations(_currentUser!.uid);
      } catch (fallbackError) {
  ...
  void updateReservation(ReservationRecord updatedReservation) {
    final index = _userReservations
        .indexWhere((r) => r.reference.id == updatedReservation.reference.id);
    if (index != -1) {
  ...
  String getWelcomeMessage() {
    return _appSettings?.welcomeMessage ??
  ...
  List<ReservationRecord> getUpcomingReservations() {
    ...
    return _userReservations
        .where((reservation) =>
            ...
            reservation.creneaux!.isAfter(now))
        .toList()
      ..sort((a, b) => a.creneaux!.compareTo(b.creneaux!));
  }
  ...
  List<ReservationRecord> getPastReservations() {
    ...
    return _userReservations
        .where((reservation) =>
            reservation.creneaux != null && reservation.creneaux!.isBefore(now))
        .toList()
      ..sort((a, b) => b.creneaux!.compareTo(a.creneaux!));
  }
